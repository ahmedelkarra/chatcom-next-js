...
export const PUT = async (req: NextRequest, { params }: { params: IParams }) => {
    ...
    try {
        await connectDB();
        const question = await Answers.findOne({ question: id });
        ...
        if (likeAction === 'plus') {
            ...
            await Answers.findOneAndUpdate({ question: id }, { $push: { likes: _id }, $pull: { dislike: _id } });
            ...
            await Answers.findOneAndUpdate({ question: id }, { $push: { dislike: _id }, $pull: { likes: _id } });
